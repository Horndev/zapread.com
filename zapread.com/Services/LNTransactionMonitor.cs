    ...
    public class LNTransactionMonitor
    {
        ...
        public void SyncNode()
        {
            using (var db = new ZapContext())
            {
                ...
                running = true;
                var website = db.ZapreadGlobals.Where(gl => gl.Id == 1)//.AsNoTracking()
                .FirstOrDefault();
                if (website == null)
                {
                ...
                int start = 63000;
                int max = 30;
                var paymentsResult = lndClient.GetPayments(include_incomplete: true, // Important for checking
 //reversed: true, // Start with most recent and work backwards
                max_payments: step);
                var paymentsResultEnd = lndClient.GetPayments(include_incomplete: true, // Important for checking
 reversed: true, // Start with most recent and work backwards
 max_payments: 1);
                ...
                while (start < max)
                {
                    foreach (var payment in paymentsResult.payments)
                    {
                        ...
                        var dbMatches = db.LightningTransactions.Where(tx => tx.PaymentRequest == invoice).ToList();
                    ...
                    //get next batch
                    paymentsResult = lndClient.GetPayments(include_incomplete: true, // Important for checking
 index_offset: start, max_payments: step);
                }
            }
        }
        ...
        public void CheckLNTransactions()
        {
            using (var db = new ZapContext())
            {
                var website = db.ZapreadGlobals.Where(gl => gl.Id == 1)//.AsNoTracking() // need to track since it will get updated
                .FirstOrDefault();
                if (website == null)
                {
                    throw new Exception("Unable to load website settings.");
                ...
                .Include(t => t.User).Include(t => t.User.Funds).OrderByDescending(t => t.TimestampCreated);
                    ...
                    foreach (var i in unpaidWithdraws)
                    {
                        ...
                        if (pmt != null)
                        {
                            ...
                            if (pmt.status == "SUCCEEDED")
                            {
                                ...
                                if (i.IsLimbo)
                                {
                                    if (i.User.Funds.LimboBalance - amount < 0)
                                    {
                                        ...
                                        Services.MailingService.SendErrorNotification(title: "Tx caused limbo to be negative. - payment verified", message: "tx.id: " + Convert.ToString(i.Id) + " Reason -1");
                                    }
                                    else
                                    {
                                ...
                                i.IsIgnored = true;
                                i.IsSettled = true;
                                Services.MailingService.SendErrorNotification(title: "Tx marked as ignored - payment verified", message: "tx.id: " + Convert.ToString(i.Id) + " Reason 0");
                            }
                            else if (pmt.status == "FAILED")
                            {
                                ...
                                i.IsIgnored = true;
                                i.IsSettled = false;
                                Services.MailingService.SendErrorNotification(title: "Tx marked as ignored - failure verified", message: "tx.id: " + Convert.ToString(i.Id) + " Reason 0");
                            }
                            ...
                            else if (i.ErrorMessage == "Error: invoice is already paid")
                            {
                                ...
                                i.IsIgnored = true;
                                i.IsSettled = false;
                                Services.MailingService.SendErrorNotification(title: "Tx marked as ignored", message: "tx.id: " + Convert.ToString(i.Id) + " Reason 1");
                            }
                            else if (i.ErrorMessage == "Error: payment is in transition")
                            {
                                // Double spend attempt stopped.  No loss of funds
                                i.IsIgnored = true;
                                Services.MailingService.SendErrorNotification(title: "Tx marked as ignored", message: "tx.id: " + Convert.ToString(i.Id) + " Reason 2");
                            }
                            else if (i.ErrorMessage == "Error: FinalExpiryTooSoon")
                            {
                                i.IsIgnored = true;
                                Services.MailingService.SendErrorNotification(title: "Tx marked as ignored", message: "tx.id: " + Convert.ToString(i.Id) + " Reason 3");
                            }
                            else if (i.ErrorMessage == "Error validating payment." || i.ErrorMessage == "Error executing payment.")
                            {
                                ...
                                i.IsSettled = true;
                                i.IsLimbo = false;
                                Services.MailingService.SendErrorNotification(title: "User withdraw limbo complete (settled)", message: "Withdraw Invoice completed limbo (payment was found)." + "\r\n invoice: " + i.PaymentRequest + "\r\n user: " + i.User.Name + "(" + i.User.AppId + ")" + "\r\n amount: " + Convert.ToString(i.Amount) + "\r\n error: " + (i.ErrorMessage ?? "null"));
                            }
                            else
                            {
                                ...
                                i.IsSettled = true;
                                Services.MailingService.SendErrorNotification(title: "Tx marked as ignored", message: "tx.id: " + Convert.ToString(i.Id) + " Reason 4");
                            }
                        }
                        else
                        {
                            ...
                            if (i.ErrorMessage == "Error: invoice is already paid")
                            {
                                // This was a duplicate payment - funds were not sent and this payment hash should only have one paid version.
                                i.IsIgnored = true;
                                Services.MailingService.SendErrorNotification(title: "Tx marked as ignored", message: "tx.id: " + Convert.ToString(i.Id) + " Reason 5");
                            }
                            else if (i.ErrorMessage == "Error: amount must be specified when paying a zero amount invoice" || i.ErrorMessage == "Error: payment attempt not completed before timeout")
                            {
                                i.IsIgnored = true;
                                if (i.User.Funds.LimboBalance - amount < 0)
                                {
                                ...
                                Services.MailingService.SendErrorNotification(title: "User withdraw limbo expired (not settled - limbo returned)", message: "Withdraw Invoice expired (payment not found). Funds released to user." + "\r\n invoice: " + i.PaymentRequest + "\r\n user: " + i.User.Name + "(" + i.User.AppId + ")" + "\r\n amount: " + Convert.ToString(i.Amount) + "\r\n error: " + (i.ErrorMessage ?? "null"));
                            }
                            else
                            {
        ...
        {
            return new LndRpcClient(host: website.LnMainnetHost, macaroonAdmin: website.LnMainnetMacaroonAdmin, macaroonRead: website.LnMainnetMacaroonRead, macaroonInvoice: website.LnMainnetMacaroonInvoice);
        }
    }
}