...
namespace zapread.com.Helpers
{
    ...
    public static class QueryHelpers
    {
        ...
        {
            IQueryable<Post> validposts = db.Posts.Where(p => !p.IsDeleted).Where(p => !p.IsDraft);
            if (userInfo != null)
            {
                var ig = userInfo.IgnoredGroups;
                validposts = validposts.Where(p => !ig.Contains(p.Group.GroupId));
                var ip = userInfo.IgnoredPosts;
                validposts = validposts.Where(p => !ip.Contains(p.PostId));
                var allLang = userInfo.ViewAllLanguages;
                if (!allLang)
                {
                    var languages = userLanguages ?? new List<string>()
                    {"en"};
                    validposts = validposts.Where(p => p.Language == null || languages.Contains(p.Language));
                }
            }
            else
            {
                ...
                {
                    validposts = db.Posts.Where(p => !p.IsDeleted).Where(p => !p.IsDraft);
                }
                else
                {
                    validposts = db.Posts.Where(p => !p.IsDeleted).Where(p => !p.IsDraft).Where(p => p.Language == null || userLanguages.Contains(p.Language));
                }
            }
        ...
        public static async Task<List<PostViewModel>> QueryActivityPostsVm(int start, int count, string userAppId)
        {
            using (var db = new ZapContext())
            {
                ...
                //        .ToListAsync()
                //        .ConfigureAwait(true);
                var userposts = db.Posts.Where(p => p.UserId.AppId == userAppId).Where(p => !p.IsDeleted).Where(p => !p.IsDraft).OrderByDescending(p => p.TimeStamp).Select(p => new PostViewModel()
                {PostTitle = p.PostTitle, Content = p.Content, PostId = p.PostId, GroupId = p.Group.GroupId, GroupName = p.Group.GroupName, IsSticky = p.IsSticky, UserName = p.UserId.Name, UserId = p.UserId.Id, UserAppId = p.UserId.AppId, UserProfileImageVersion = p.UserId.ProfileImage.Version, Score = p.Score, TimeStamp = p.TimeStamp, TimeStampEdited = p.TimeStampEdited, IsNSFW = p.IsNSFW, IsNonIncome = p.IsNonIncome, ViewerIsFollowing = p.FollowedByUsers.Select(v => v.AppId).Contains(userAppId), ViewerIsMod = p.Group.Moderators.Select(m => m.AppId).Contains(userAppId), ViewerUpvoted = p.VotesUp.Select(v => v.AppId).Contains(userAppId), ViewerDownvoted = p.VotesDown.Select(v => v.AppId).Contains(userAppId), ViewerIgnoredUser = p.UserId.AppId == userAppId ? false : p.UserId.IgnoredByUsers.Select(u => u.AppId).Contains(userAppId), ViewerIgnoredPost = p.UserId.AppId == userAppId ? false : p.IgnoredByUsers.Select(u => u.AppId).Contains(userAppId), CommentVms = p.Comments.Select(c => new PostCommentsViewModel()
                {PostId = p.PostId, CommentId = c.CommentId, Text = c.Text, Score = c.Score, IsReply = c.IsReply, IsDeleted = c.IsDeleted, TimeStamp = c.TimeStamp, TimeStampEdited = c.TimeStampEdited, UserId = c.UserId.Id, UserName = c.UserId.Name, UserAppId = c.UserId.AppId, ProfileImageVersion = c.UserId.ProfileImage.Version, ViewerUpvoted = c.VotesUp.Select(v => v.AppId).Contains(userAppId), ViewerDownvoted = c.VotesDown.Select(v => v.AppId).Contains(userAppId), ViewerIgnoredUser = c.UserId.AppId == userAppId ? false : c.UserId.IgnoredByUsers.Select(u => u.AppId).Contains(userAppId), ParentCommentId = c.Parent == null ? 0 : c.Parent.CommentId, ParentUserId = c.Parent == null ? 0 : c.Parent.UserId.Id, ParentUserAppId = c.Parent == null ? "" : c.Parent.UserId.AppId, ParentUserName = c.Parent == null ? "" : c.Parent.UserId.Name, }), });
                ...
                //    .Take(count)
                //    .ToList();
                var activityposts = await userposts.Skip(start).Take(count).ToListAsync().ConfigureAwait(true);
        ...
        public static IQueryable<PostQueryInfo> OrderPostsByActive(IQueryable<Post> validposts)
        {
            ...
            var sposts = validposts.Where(p => !p.IsDeleted).Where(p => !p.IsDraft).Select(p => new
            {
            p, s = (Math.Abs((double)p.Comments.Count) < 1.0 ? 1.0 : 100000.0 * Math.Abs((double)p.Comments.Count)), // Max (|x|,1)                                                           
 }).Select(p => new
            {
            p.p, order = SqlFunctions.Log10(p.s), sign = p.p.Comments.Count >= 0.0 ? 1.0 : -1.0, // Sign of s
 dt = 1.0 * DbFunctions.DiffSeconds(scoreStart, p.p.TimeStamp), // time since start
 }).Select(p => new PostQueryInfo()
            {p = p.p, RootComments = p.p.Comments.Where(c => !c.IsReply).OrderByDescending(c => c.Score).Select(c => c.CommentId), hot = 0, active = (p.sign * p.order) + (p.dt / 2000000.0) // Reduced time effect
        ...
        {
            IQueryable<PostQueryInfo> sposts = validposts.Where(p => p.Score > -50).Select(p => new PostQueryInfo()
            {p = p, hot = 0, RootComments = p.Comments.Where(c => !c.IsReply).OrderByDescending(c => c.Score).Select(c => c.CommentId), });
            // filter by group
            if (groupId > 0)
            {
            ...
            if (stickyPostOnTop)
            {
                sposts = sposts.OrderByDescending(p => new
                {
                p.p.IsSticky, p.p.TimeStamp
                });
            }
            else
            {
...
#pragma warning restore CS1591 // Missing XML comment for publicly visible type or member
        ...
        public static IQueryable<PostQueryInfo> OrderPostsByScore(IQueryable<Post> validposts)
        {
            ...
            //var days60 = 5184000.0;
            //var days30 = 2592000.0;
            var sposts = validposts.Where(p => !p.IsDeleted).Where(p => !p.IsDraft).Select(p => new
            {
                p, // Includes the sum of absolute value of comment scores
                //cScore = p.Comments.Any() ? p.Comments.Where(c => !c.IsDeleted).Sum(c => Math.Abs((double)c.Score) < 1.0 ? 1.0 : Math.Abs((double)c.Score)) : 1.0
                cScoreAbs = p.Comments.Any() ? Math.Abs(p.Comments.Where(c => !c.IsDeleted).Sum(c => c.Score > 0 ? c.Score : 0)) : 0.0,
            }).Select(p => new
            {
                p.p,
                p.cScoreAbs,
                sign = p.p.Score > 0.0 ? 1.0 : -1.0, // Sign of s
                sAbs = Math.Abs((double)p.p.Score),//(Math.Abs((double)p.p.Score) < 1.0 ? 1.0 : Math.Abs((double)p.p.Score)), // Max (|x|,1)                                                           
            }).Select(p => new
            {
                p.p,
                order1 = SqlFunctions.SquareRoot(p.sign > 0 ? p.sAbs / 50000.0 : p.sAbs / 1000.0),//SqlFunctions.Log10(p.s),
                order2 = SqlFunctions.SquareRoot(p.cScoreAbs / 50000.0),//SqlFunctions.Log10(p.cScore < 1.0 ? 1.0 : p.cScore), // Comment scores
                p.sign,
                dt = -1.0 * DbFunctions.DiffSeconds(DateTime.Now, p.p.TimeStamp), // time since start
            }).Select(p => new PostQueryInfo()
            {
                p = p.p,
                //order1 = p.order1,
                ...
                hot = (SqlFunctions.Exp(-1.0 * p.dt.Value / 5184000.0) * (p.sign * 100.0 * p.order1 + 20.0 * p.order2)) + (100.0 * SqlFunctions.Exp(-1.0 * p.dt.Value / 2592000.0)),  // 60 days in seconds = 5184000
                RootComments = p.p.Comments.Where(c => !c.IsReply).Select(c => c.CommentId),
        ...
        public static async Task<List<PostViewModel>> QueryPostsVm(int start, int count, IQueryable<PostQueryInfo> postquery, PostQueryUserInfo userInfo = null, int numComments = 5, bool limitComments = false)
        {
            ...
            if (limitComments)
            {
                var sposts = await postquery.Skip(start).Take(count).Select(p => new PostViewModel()
                {
                    Hot = p.hot ?? 0,
