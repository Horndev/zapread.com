...
namespace zapread.com.Controllers
{
    ...
    public class MailerController : Controller
    {
        ...
        public async Task<ActionResult> MailerNewPost(int id)
        {
            ...
            {
                var vm = db.Posts.Where(p => p.PostId == id).Select(p => new NewPostEmail()
        ...
        public async Task<ActionResult> MailerNewComment(int? id)
        {
            ...
            using (var db = new ZapContext())
            {
                var vm = await db.Comments.Where(cmt => cmt.CommentId == id).Select(c => new Models.Email.PostCommentEmail()
                {CommentId = c.CommentId, Score = c.Score, Text = c.Text, UserId = c.UserId.Id, UserName = c.UserId.Name, UserAppId = c.UserId.AppId, ProfileImageVersion = c.UserId.ProfileImage.Version, PostTitle = c.Post == null ? "" : c.Post.PostTitle, PostId = c.Post == null ? 0 : c.Post.PostId, }).FirstOrDefaultAsync().ConfigureAwait(true);
                if (vm == null)
                {
        ...
        public async Task<ActionResult> MailerWeeklySummary()
        {
            using (var db = new ZapContext())
            {
                ...
                var refCode = await db.Users.Where(u => u.AppId == userAppId).Select(u => u.ReferralCode).FirstOrDefaultAsync().ConfigureAwait(true);
                if (refCode == null)
                {
                ...
                var vm = await db.Users.Where(u => u.AppId == userAppId).Select(u => new WeeklySummaryEmail()
                ...
                {GroupName = o.GroupName, GroupId = i.GroupId, AmountEarned = i.Amount}).OrderByDescending(c => c.AmountEarned).Take(3).ToList(), }).FirstOrDefaultAsync();
        ...
        public async Task<ActionResult> MailerCommentReply(int? id)
        {
            ...
            BackgroundJob.Enqueue<MailingService>(methodCall: x => x.MailPostCommentReply(id.Value, // commentId
 true // isTest
            ));
            using (var db = new ZapContext())
            {
                var vm = await db.Comments.Where(cmt => cmt.CommentId == id).Select(c => new PostCommentReplyEmail()
                {CommentId = c.CommentId, Score = c.Score, Text = c.Text, UserId = c.UserId.Id, UserName = c.UserId.Name, UserAppId = c.UserId.AppId, ProfileImageVersion = c.UserId.ProfileImage.Version, PostTitle = c.Post == null ? "" : c.Post.PostTitle, PostId = c.Post == null ? 0 : c.Post.PostId, ParentCommentId = c.Parent == null ? 0 : c.Parent.CommentId, ParentUserId = c.Parent == null ? 0 : c.Parent.UserId.Id, ParentUserAppId = c.Parent == null ? "" : c.Parent.UserId.AppId, ParentUserProfileImageVersion = c.Parent == null ? 0 : c.Parent.UserId.ProfileImage.Version, ParentUserName = c.Parent == null ? "" : c.Parent.UserId.Name, ParentCommentText = c.Parent == null ? "" : c.Parent.Text, ParentScore = c.Parent == null ? 0 : c.Parent.Score, }).FirstOrDefaultAsync().ConfigureAwait(true);
        ...
        public async Task<ActionResult> MailerNewChat(int id)
        {
            ...
            using (var db = new ZapContext())
            {
                var vm = await db.Messages.Where(m => m.Id == id).Select(m => new NewChatEmail()
                {FromName = m.From.Name, FromAppId = m.From.AppId, FromProfileImgVersion = m.From.ProfileImage.Version, IsReceived = true, Content = m.Content}).FirstOrDefaultAsync().ConfigureAwait(true);
        ...
        protected string RenderViewToString(string viewName, object model)
        {
            ...
            {
                ViewEngineResult viewResult = ViewEngines.Engines.FindView(ControllerContext, "~/Views/Mailer/" + viewName + ".cshtml", null);
