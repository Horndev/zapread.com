...
namespace zapread.com.API
{
    ...
    public class AdminController : ApiController
    {
        ...
        public async Task<ZapReadResponse> CheckOnline(int userId)
        {
            ...
            {
                var user = await db.Users.Where(u => u.Id == userId).SingleOrDefaultAsync().ConfigureAwait(false);
                var jobId = BackgroundJob.Schedule<UserState>(methodCall: x => x.UserOffline(user.AppId, user.Name, DateTime.UtcNow), delay: TimeSpan.FromMinutes(1));
        ...
        public async Task<IHttpActionResult> GrantReactionAll(GrantReactionParameters value)
        {
            ...
            using (var db = new ZapContext())
            {
                var reaction = await db.Reactions.Where(r => r.ReactionId == value.ReactionId).FirstOrDefaultAsync().ConfigureAwait(true);
        ...
        public async Task<IHttpActionResult> GrantReaction(GrantReactionParameters value)
        {
            ...
            using (var db = new ZapContext())
            {
                var alreadyHasReaction = await db.Users.Where(u => u.AppId == value.UserAppId).Select(u => u.AvailableReactions.Any(r => r.ReactionId == value.ReactionId)).FirstOrDefaultAsync().ConfigureAwait(true);
                ...
                var user = await db.Users.Where(u => u.AppId == value.UserAppId).FirstOrDefaultAsync().ConfigureAwait(true);
                var reaction = await db.Reactions.Where(r => r.ReactionId == value.ReactionId).FirstOrDefaultAsync().ConfigureAwait(true);
        ...
        public async Task<IHttpActionResult> AddReaction(AddReactionParameters value)
        {
            ...
            using (var db = new ZapContext())
            {
                ...
                {ReactionIcon = value.ReactionIcon, ReactionName = value.ReactionName, Description = value.ReactionDescription};
                db.Reactions.Add(newReaction);
                await db.SaveChangesAsync();
                return Ok(new AddReactionResponse()
                {Reaction = newReaction, success = true});
            }
        }
        ...
        public async Task<IHttpActionResult> GetReactions()
        {
            using (var db = new ZapContext())
            {
                var reactions = await db.Reactions.Select(r => new ReactionItem()
                {Description = r.Description, ReactionIcon = r.ReactionIcon, ReactionId = r.ReactionId, ReactionName = r.ReactionName, }).ToListAsync();
        ...
        public async Task<IHttpActionResult> GetAccountingLiability(int year, int month)
        {
            using (var db = new ZapContext())
            {
                try
                {
                    var q = await db.Users.Where(u => u.Funds != null).Select(u => u.Funds).GroupBy(u => u.IsWithdrawLocked).Select(g => new
                    {
                    isWithdrawLocked = g.Key, balance = g.Sum(f => f.Balance), limbo = g.Sum(f => f.LimboBalance), count = g.Count()}).ToListAsync().ConfigureAwait(false);
                    var header = new Row()
                    {Cells = new List<Cell>()};
                    ...
                    foreach (var entry in q)
                    {
                        ...
                        {value = entry.isWithdrawLocked ? "Locked" : "Normal"});
                        row.Cells.Add(new Cell()
                        {value = entry.count});
                        row.Cells.Add(new Cell()
                        {value = entry.balance});
                        ...
                        {value = entry.limbo});
                        row.Cells.Add(new Cell()
                        {value = entry.balance + entry.limbo});
        ...
        public async Task<IHttpActionResult> GetAccountingRevenueByDay(int year, int month)
        {
            using (var db = new ZapContext())
            {
                var website = await db.ZapreadGlobals.Include(z => z.EarningEvents).FirstOrDefaultAsync(i => i.Id == 1).ConfigureAwait(false);
                if (website == null)
                {
                ...
                var transactionsByday = website.EarningEvents.Where(e => e.TimeStamp.Value.Year == year && e.TimeStamp.Value.Month == month).OrderBy(e => e.TimeStamp).GroupBy(e => e.TimeStamp.Value.Day).Select(e => new
                {
                day = e.Key, total = e.Sum(v => v.Amount), average = e.Average(v => v.Amount), count = e.Count(), posts = e.Count(v => v.OriginType == 0), comments = e.Count(v => v.OriginType == 1), }).ToList();
                var header = new Row()
                {Cells = new List<Cell>()};
                header.Cells.Add(new Cell()
                {value = "Year"});
                ...
                foreach (var transaction in transactionsByday)
                {
                    ...
                    row.Cells.Add(new Cell()
                    {value = transaction.average});
                    row.Cells.Add(new Cell()
                    {value = transaction.posts});
                    row.Cells.Add(new Cell()
                ...
                {value = totalEvents});
                footer.Cells.Add(new Cell()
                {value = totalEvents > 0 ? totalEarned / totalEvents : 0});
                footer.Cells.Add(new Cell()
                {value = totalPosts});
                footer.Cells.Add(new Cell()
                {value = totalComments});
