...
namespace zapread.com.API
{
    ...
    public class TagController : ApiController
    {
        ...
        public async Task<IHttpActionResult> GetPosts(GetTagPostsParameters req)
        {
            if (req == null)
                return BadRequest();
            if (req.TagId.HasValue && req.TagId < 1)
                return BadRequest();
            if (!req.TagId.HasValue && String.IsNullOrEmpty(req.TagName))
                return BadRequest();
            ...
            using (var db = new ZapContext())
            {
                ...
                if (!req.TagId.HasValue)
                {
                    ...
                    var tagIdRes = await db.Tags.Where(g => g.TagName == nameQuery).Select(g => new
                    {
                    g.TagId
                    }).FirstOrDefaultAsync();
                    if (tagIdRes == null)
                ...
                var tagPosts = QueryHelpers.OrderPostsByNew(validposts: validposts, tagId: tagId, stickyPostOnTop: false);
                var postsVm = await QueryHelpers.QueryPostsVm(start: BlockNumber * BlockSize, count: BlockSize, postquery: tagPosts, userInfo: userInfo, limitComments: true).ConfigureAwait(true);
                ...
                {HasMorePosts = tagPosts.Count() >= BlockNumber * BlockSize, Posts = postsVm, success = true, };
                return Ok(response);
            }
        }
        ...
        public async Task<IHttpActionResult> GetTagInfo(GetTagInfoRequest req)
        {
            ...
            using (var db = new ZapContext())
            {
                var tag = await db.Tags.Where(t => t.TagName == req.TagName).Select(t => new TagItem()
                {id = t.TagId, value = t.TagName, count = t.Posts.Count}).FirstAsync();
                return Ok(new GetTagInfoResponse()
                {Tag = tag, IsLoggedIn = User.Identity.GetUserId() != null, success = true});
            }
        }
        ...
        public async Task<IHttpActionResult> GetTags()
        {
            using (var db = new ZapContext())
            {
                var tags = await db.Tags.OrderByDescending(t => t.Posts.Count + t.Comments.Count).Where(t => t.Posts.Where(p => !p.IsDeleted && !p.IsDraft).Count() > 0 || t.Comments.Where(c => !c.IsDeleted && !c.Post.IsDeleted && !c.Post.IsDraft).Count() > 0).Take(100).Select(t => new TagItem()
                {value = t.TagName, id = t.TagId, count = t.Posts.Where(p => !p.IsDeleted && !p.IsDraft).Count(), CommentCount = t.Comments.Where(c => !c.IsDeleted && !c.Post.IsDeleted && !c.Post.IsDraft).Count()}).ToListAsync();
                tags.ForEach(t => t.link = "/tag/" + Uri.EscapeDataString(t.value.CleanUnicode().Trim()).Replace("%EF%BB%BF", "") + "/");
                return Ok(new GetTagsResponse()
                {success = true, Tags = tags});
            }
        }
        ...
        public async Task<IHttpActionResult> MentionTags(MentionTagRequest req)
        {
            ...
            using (var db = new ZapContext())
            {
                ...
                var tagExists = await db.Tags.AnyAsync(t => t.TagName.ToUpper() == searchTerm.ToUpper());
                var tags = await db.Tags.Where(t => t.TagName.Contains(searchTerm)).OrderByDescending(t => t.Posts.Count()).Select(u => new TagItem()
                {id = u.TagId, value = u.TagName}).Take(10).ToListAsync().ConfigureAwait(false);
                if (!tagExists)
                {
                    tags = tags.Prepend(new TagItem()
                    {id = -1, value = searchTerm, newtag = true}).ToList();
                }
